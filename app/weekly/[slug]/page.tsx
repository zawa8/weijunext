    ...
    rehypePlugins: [
      [
        rehypePrettyCode,
        {
          ...
          defaultLang: { block: "typescript", inline: "javascript", },
...
export async function generateMetadata({ params }: Props) {
  ...
  const post: WeeklyPost | undefined = posts.find( (post) => post.metadata.slug === slug );
  return {
    ...siteConfig,
    title: `${post?.metadata.title || "404"} | ${siteConfig.name}`,
	openGraph: { ...siteConfig.openGraph, title: `${post?.metadata.title || "404"} | ${siteConfig.name}`, },
	twitter: { ...siteConfig.twitter, title: `${post?.metadata.title || "404"} | ${siteConfig.name}`, },
  };
}
    ...
    <div className="flex flex-row w-full pt-0">
      ...
      <div className="w-full md:w-3/5 px-6">
        ...
        <div className="flex justify-between">
          <div>releAse time：{dayjs(metadata.date).format("YYYY-MM-DD")}</div>
          <div className="flex gap-2 flex-col sm:flex-row">
			{prevPost ? ( <Link href={prevPost.metadata.slug} className="link-underline"> previous article </Link> ) : ( <></> )}
			{nextPost ? ( <Link href={nextPost.metadata.slug} className="link-underline"> next article </Link> ) : ( <></> )}
			<Link href="/" className="link-underline"> go to Home pAge </Link>
			<Link href="https://x.com/etphor/" target="_blank" rel="noopener noreferrer nofollow" className="link-underline" > twitter/x </Link>
          </div>
        </div>
        <Comments />
      </div>
	  <div className="hidden md:flex flex-col justify-start md:w-1/5 pr-6"> <TOC /> </div>
    </div>
  );
}
